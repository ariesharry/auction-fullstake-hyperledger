...
func (c *Contract) Issue(ctx TransactionContextInterface, issuer string, paperNumber string, issueDateTime string, maturityDateTime string, quantity int, commodity string, lotNumber string, quality string, producer string, certification string, portOfLoading string, deliveryConditions string) (*CommercialPaper, error) {
	paper := CommercialPaper{PaperNumber: paperNumber, Issuer: issuer, IssueDateTime: issueDateTime, Commodity: commodity, LotNumber: lotNumber, Quantity: quantity, Quality: quality, Producer: producer, Certification: certification, PortOfLoading: portOfLoading, DeliveryConditions: deliveryConditions, MaturityDateTime: maturityDateTime, Owner: issuer}
	paper.SetIssued()
...
func (c *Contract) Redeem(ctx TransactionContextInterface, issuer string, paperNumber string, redeemingOwner string, redeenDateTime string) (*CommercialPaper, error) {
	...
	if paper.IsRedeemed() {
		return nil, fmt.Errorf("Paper %s:%s is already DELIVERED", issuer, paperNumber)
	}
