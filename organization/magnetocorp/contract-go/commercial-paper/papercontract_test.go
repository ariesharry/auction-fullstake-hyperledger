...
func TestIssue(t *testing.T) {
	...
	expectedPaper := CommercialPaper{PaperNumber: "somepaper", Issuer: "someissuer", IssueDateTime: "someissuedate", Commodity: "somecommodity", LotNumber: "someilotNumber", Quantity: 1000, Quality: "someQuality", Producer: "someProducer", Certification: "someCertification", PortOfLoading: "somePortOfLoading", DeliveryConditions: "someDeliveryConditions", MaturityDateTime: "somematuritydate", Owner: "someissuer", state: 1}
...
func TestBuy(t *testing.T) {
	...
	assert.EqualError(t, err, "Paper someissuer:somepaper is not trading. Current state = DELIVERED")
...
func TestRedeem(t *testing.T) {
	...
	assert.EqualError(t, err, "Paper someissuer:somepaper is already DELIVERED", "should error when paper already DELIVERED")
	assert.Nil(t, paper, "should not return paper when errors as already DELIVERED")
	...
	resetPaper(wsPaper)
	paper, err = contract.Redeem(ctx, "someissuer", "somepaper", "someowner", "2021-12-10:10:00")
	assert.Nil(t, err, "should not error on good DELIVERED")
	assert.True(t, paper.IsRedeemed(), "should return DELIVERED paper")
	assert.Equal(t, sentPaper, paper, "should update same paper as it returns in the world state")
}
